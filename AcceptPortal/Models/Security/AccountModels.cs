    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Login(string returnUrl)
        {
            ...
            if (TempData.ContainsKey("ProjectIdInvitationUserName") && TempData.ContainsKey("ProjectIdInvitation"))
            {
                ...
                if (TempData.ContainsKey("ProjectIdInvitationUserNameEval") && TempData.ContainsKey("ProjectIdInvitationEval"))
                {
                    ViewBag.UserName = (string)TempData["ProjectIdInvitationUserNameEval"];
                    ViewBag.ProjectId = TempData["ProjectIdInvitationEval"];
                    TempData.Clear();
                    TempData.Add("ProjectIdInvitationUserNameEval", ViewBag.UserName);
                    TempData.Add("ProjectIdInvitationEval", ViewBag.ProjectId);
                }
                #endregion
            #endregion
        ...
        public ActionResult Login(LoginModel model, string returnUrl)
        {
            ...
            if (ModelState.IsValid && Membership.ValidateUser(model.UserName, model.Password))
            {
                ...
                if (Url.IsLocalUrl(returnUrl) && returnUrl.Length > 1 && returnUrl.StartsWith("/") && !returnUrl.StartsWith("//") && !returnUrl.StartsWith("/\\"))
                {
                    ...
                    if (System.Web.HttpContext.Current.Request.Cookies["AceeptRedirectionEval"] != null)
                    {
                        HttpCookie myCookie = System.Web.HttpContext.Current.Request.Cookies["AceeptRedirectionEval"];
                        string projectId = myCookie["ProjectIdEval"];

                        //delete redirection cookie.
                        var c = new HttpCookie("AceeptRedirectionEval");
                        c.Expires = DateTime.Now.AddDays(-1);
                        Response.Cookies.Add(c);

                        return RedirectToAction("EvaluateProject", new RouteValueDictionary(new { controller = "Evaluation", action = "EvaluateProject", Id = projectId }));
                    }
                    else
                        if (TempData.ContainsKey("ProjectIdInvitationEval"))
                        {
                            var projectId = TempData["ProjectIdInvitationEval"];
                            TempData.Clear();
                            return RedirectToAction("EvaluateProject", new RouteValueDictionary(new { controller = "Evaluation", action = "EvaluateProject", Id = projectId.ToString() }));
                        }
                        else
        ...
        public ActionResult Register()
        {
            ...
            #region EvaluationInternal
                ...
                if (TempData.ContainsKey("ProjectIdInvitationUserNameEval"))
                    ViewBag.UserName = (string)TempData["ProjectIdInvitationUserNameEval"];
            #endregion
