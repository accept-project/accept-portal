...
(function ($) {
    ...
    $.fn.Accept = function (options) {
        ...
        var settings = $.extend({
            ...
            'languageUi': 'en',
            'dialogHeight': 'auto',//320,
            'dialogWidth': 'auto', //480
            'requestFormat': 'TEXT',
		...
		var SubmitFlagAudit = function (flagAudit) 			
		{
                ...
                {
					//var call = settings.AcceptServerPath + "/Core/AuditFlagJsonP?=" + this.sessioncodeid + "&AuditTypeId=" + this.audittypeid + "&RuleName=" + this.rulename + "&AuditContext=" + this.context + "&TextBefore=" + this.textbefore + "&TextAfter=" + this.textafter + "&StartTime=" + this.starttime.toUTCString() + "&EndTime=" + this.endtime.toUTCString() + "&callback=?";				
					$.ajax({
						type: 'GET',
						url: call,
						dataType: 'jsonp',
        ...
        var initAccept = function () 
		{
            ...
            if (settings.requestFormat == 'TEXT')
                ...
                if (settings.requestFormat == 'HTML') 
				{
                    ...
                    acceptTextContext = CleanText($('#htmlPlaceHolderDiv_' + acceptDialogId).text());
                }
            ...
            +'</table>'            
            +'</div>');												
			//<img alt="X" src="' + settings.imagesPath + '/applyimage.png" />
			//<img alt="X" src="' + settings.imagesPath + '/closeimage.png" />								 							
        ...
        function CreateContextMenu(menuname) {
            ...
            $('#' + acceptContainerId + ' SPAN.highlight').each(function (index) {//$('#divgrammar P SPAN').each(function (index) {
                ...
                if (!$(this).data('events')) {
					...
					// document.getElementById(menuname).addEventListener('mouseleave',onMouseOut,true);								
                                    ...
                                    case "sug_":
                                        {
                                            ...
                                            var suggestionToUse = $(this).text(); //this.childNodes[2].data;
                                            ...
                                            if (clickedLiSpllited[5] == "gr") 
											{
                                                ...
                                                $('#' + acceptContainerId).find('span[id^="spncontext_' + clickedLiSpllited[3] + '_' + clickedLiSpllited[4] + '"]').each(function (index) 
												{
													...
													//sendAuditFlag = function(startIndex, endIndex, contextFlagged, usedflag, usedFlagContextMenuSelector)
                                            ...
                                            else 
											{
                                                ...
                                                if (spnContext != null) 
												{
													...
													//remove highlight
													//$(spnContext).replaceWith($(spnContext).text());
                                                     removeKeepChildren($(spnContext));
													//$(spnContext).removeClass();
													//$(spnContext).unbind();
 ...
 function FixAll() {
        ...
        function PrepareTextOutput() {
            ...
            if ($("#" + acceptContainerId).text().length == 0) {
                ...
                if (settings.requestFormat == "TEXT") {
                    ...
                    //$("#" + acceptContainerId).text(decodeURIComponent(acceptTextContext));

                    //$('#htmlPlaceHolderDiv_' + acceptDialogId).find('P').each(function () {
                    //    var pnode = $(this).text().replace('@ACCEPTENDLINE@', '');
                    //    $(this).text(pnode);
                    //});
		...
		var CleanText = function (text) {
            ...
            if (settings.requestFormat == 'TEXT') {
                ...
                //cleanedText = htmlEncode($.trim(text.replace(/&nbsp;/gi, ' ')));               
                //              $('#htmlPlaceHolderDiv_' + acceptDialogId).find('P').each(function () {                  
                //                    var pnode = $(this).text() + '@ACCEPTENDLINE@';                  
                //                    $(this).text(pnode);
                //              });
		...
		function GetData(text, withGlobalSession)
		{
			if(withGlobalSession)
			{
				return{
				//"User": "david",
				//"Password":"David_Silva@symantec.com",
					...
					if(globalSessionId != null && globalSessionId.length > 0)
					 {
						...
						//jsonData = '{"User":"david", "Password":"David_Silva@symantec.com","GlobalSessionId":"'+globalSessionId+'", "Text":"' + text + '", "Language":"' + settings.Lang + '", "Rule":" ' + settings.Rule + '", "Grammar":"' + checkGrammar + '", "Spell":"' + checkSpelling + '", "Style":"' + checkStyle + '", "RequestFormat":"' + settings.requestFormat + '"}';
						//call = settings.AcceptServerPath + "/Core/AcceptRequestJsonP?User=david&Password=David_Silva@symantec.com&GlobalSessionId="+globalSessionId+"&Text=" + text + "&Language=" + settings.Lang + "&Rule=" + settings.Rule + "&Grammar=" + checkGrammar + "&Spell=" + checkSpelling + "&Style=" + checkStyle + "&RequestFormat=" + settings.requestFormat + "&callback=?";										
					 }	
					 else
					 {	
						...
						//jsonData = '{"User":"david", "Password":"David_Silva@symantec.com", "Text":"' + text + '", "Language":"' + settings.Lang + '", "Rule":" ' + settings.Rule + '", "Grammar":"' + checkGrammar + '", "Spell":"' + checkSpelling + '", "Style":"' + checkStyle + '", "RequestFormat":"' + settings.requestFormat + '"}';
						//call = settings.AcceptServerPath + "/Core/AcceptRequestJsonP?User=david&Password=David_Silva@symantec.com&Text=" + text + "&Language=" + settings.Lang + "&Rule=" + settings.Rule + "&Grammar=" + checkGrammar + "&Spell=" + checkSpelling + "&Style=" + checkStyle + "&RequestFormat=" + settings.requestFormat + "&callback=?";		
					 }
            ...
            $("#" + acceptDialogId + "").dialog({ width: settings.dialogWidth, height: settings.dialogHeight,modal:settings.isModal, maxWidth:settings.placeHolderMaxWidth,minHeight:settings.placeHolderMinHeight,minWidth:settings.placeHolderMinWidth,               
                ...
                close: function (event, ui) {
					...
					if(globalSessionId != null && globalSessionId.length > 0)
					{
						...
						{					
							acceptTextContext = CleanText($('#htmlPlaceHolderDiv_' + acceptDialogId).text());  //escape($('#' + acceptContainerId).html()                       					
